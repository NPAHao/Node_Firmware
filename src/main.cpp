...
#define TOUCH_PIN_2         27
#define TOUCH_PIN_3         26
#define TOUCH_PIN_4         25
...
void analog_config_task(void *pvParameters) {
    ...
    {
        String analog;
        xQueueReceive(analog_config_queue, &analog, portMAX_DELAY);
        doc["node"] = node;
        doc["analog"] = analog;
...
void analog_value_task(void *pvParameters) {
    ...
    DynamicJsonDocument doc(ESP_NOW_MAX_DATA_LEN);
    doc["object"] = String("analog value");
    doc["node"] = node;
    { // get config
        { //analog config 1
            pref.begin("analog 1", true);
...
void dht11_task(void *pvPameters) {
    DynamicJsonDocument doc(ESP_NOW_MAX_DATA_LEN);
    doc["object"] = String("dht11");
    doc["node"] = node;
...
void process_espnow_packet_task(void *pvParameters) 
{
    ...
    while (1)
    {
        ...
        if(check_except) {
            ...
            } else if(object == String("set analog config")) 
            {
                xQueueSend(set_analog_config_queue, &packet, portMAX_DELAY);
            } else if(object == String("get analog value")) 
            {
                xQueueSend(get_analog_value_queue, &packet, portMAX_DELAY);
            } else {
                memcpy(packet.mac, hub_mac, ESP_NOW_ETH_ALEN);
                xQueueSend(espnow_send_queue, &packet, portMAX_DELAY);
            }
...
void setup() 
{
    ...
    xTaskCreate(dht11_task, "dht11 task", 4096, NULL, 10, NULL);
